...
class CJustPaysendQueue;
class CJustPaysendBroadcastTx;
...
extern std::vector<CJustPaysendQueue> vecJustPaysendQueue;
extern std::string strMasterNodePrivKey;
extern map<uint256, CJustPaysendBroadcastTx> mapJustPaysendBroadcastTxes;
extern CActiveMasternode activeMasternode;

//specific messages for the JustPaysend protocol
...
int GetInputJustPaysendRounds(CTxIn in, int rounds=0);
...
public:
    ...
    void InitCollateralAddress(){
        std::string strAddress = "";
            strAddress = "LYPMPbL6cyfCaThp7S1CcmRUULpdunkt9L";
    ...
    // Passively run JustPaysend in the background according to the configuration in settings (only for QT)
    bool DoAutomaticDenominating(bool fDryRun=false, bool ready=false);
    bool PrepareJustPaysendDenominate();


    // check for process in JustPaysend
    void Check();
    ...
    void SendJustPaysendDenominate(std::vector<CTxIn>& vin, std::vector<CTxOut>& vout, int64_t amount);
    ...
    // get the denominations for a specific amount of JustPay.
